...
fichero 8 IRPF (texto plano, ~5 GB)          módulo inmobiliario (3 TXT)
   ...
   [02]  informe en R  ──►  informe_arrendadores_vivienda.xlsx (o CSVs)
        │
        ├─ [03] errores territoriales ──► errores_territoriales.xlsx
        └─ [04] beneficio por modalidad ──► beneficio_modalidades.xlsx
```
...
# 4) Generar el informe y sus dos analisis complementarios.
#    Los tres, en la MISMA sesion de R y en este orden: 03 y 04 reutilizan
#    los objetos que deja el 02 en memoria.
Rscript -e 'source("scripts/02_informe_arrendadores.R", encoding="UTF-8");
            source("scripts/03_errores_territoriales.R", encoding="UTF-8");
            source("scripts/04_beneficio_modalidades.R", encoding="UTF-8")'
```
...
**Regla de oro del `01c`: solo añade columnas que falten.** Nunca reescribe las
que ya puso el `01b`, porque de ellas depende la clasificación del informe:
ampliar el panel no puede alterar los recuentos ya validados. Si necesitas
regenerar alguna, vuelve a construir el panel con el `01b` en vez de
sobrescribir; `scripts/05_diagnostico_panel.R` compara el panel con su copia
`.bak` para ver qué columnas han cambiado.

## 🎯 ¿Cuánto error asumo por territorio? (script 03)

Tras generar el informe, `scripts/03_errores_territoriales.R` cuantifica los
tres errores de la estimación de vivienda no habitual en cada nivel
(nacional → CCAA → provincia → capitales): **E1 clasificación** (desviación
frente al ancla oficial AEAT donde existe), **E2 localización** (viviendas sin
cruce con el módulo inmobiliario, con cota de reasignación por provincia) y
**E3 muestreo** (IC 95 % e CV por bootstrap de declarantes). Produce
`errores_territoriales.xlsx` con un semáforo de publicabilidad por celda.
Uso: `source("02_informe_arrendadores.R"); source("03_errores_territoriales.R")`
(réplicas configurables con la variable de entorno `B_BOOT`, 300 por defecto).

## 💶 ¿Es más lucrativo, no solo más caro? (script 04)

`scripts/04_beneficio_modalidades.R` lleva la comparación de los ingresos al
**beneficio**: ingresos → beneficio bruto (rendimiento neto fiscal, c.149, tras
todos los gastos deducibles según la fórmula oficial del Modelo 100) →
beneficio neto tras IRPF al **tipo marginal deducido del propio panel** (por CCAA y ejercicio, a partir de las cuotas íntegras del fichero 4),
incorporando las dos asimetrías fiscales: la reducción del art. 23.2 LIRPF
(60 % en 2023, solo vivienda habitual; base gravada = c.154) y la imputación
de rentas por los días no alquilados **tal y como la declaró cada
contribuyente** (c.89 y c.87, sin simular). Todo por vivienda entera, en euros
al año y por día, con desglose de gastos por casilla, uni/multi y los
equivalentes netos de las métricas de M2, más **7 gráficos** (`M6_g1`…`M6_g7`)
que replican en beneficio los que el informe presenta en ingresos: escalera
ingresos→bruto→neto, beneficio anual y por día, margen sobre ingresos, carga
fiscal y valor de la reducción, uni vs multi, y desglose de gastos. Requiere haber pasado el `01c`
(versión 3, que añade las casillas 104, 116, 117, 132, 146-148, 151, 152, 154,
87 y 89 en 2023, y sus equivalentes P59-P74 en 2016). Incluye la **comparativa
temporal 2016-2023** del diferencial en ingresos frente al diferencial en
beneficio bruto y neto, gravando cada ejercicio con su propia escala del IRPF. Uso: tras el 02, `source("04_beneficio_modalidades.R")`.
