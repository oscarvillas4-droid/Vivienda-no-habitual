...
add_x(grafico("X2_precio_dia_vs_habitual", mar = c(7.5, 7, 5.6, 2.5), expr = {
  # Dias CENSALES por referencia catastral (decision D10): son los que usan la
  # validacion frente a la AEAT y el resto del informe. Usar aqui los dias del
  # declarante muestreado daria un euro/dia distinto al de las demas tablas.
  b <- copy(base)
  b[, d2 := if ("DIAS_RC" %in% names(b)) {
      x <- suppressWarnings(as.numeric(DIAS_RC))
      fifelse(is.na(x) | x <= 0, dias_ef, pmin(x, 365))
    } else dias_ef]
  b <- b[!is.na(d2) & d2 > 0 & !is.na(ing_viv)]
  r <- b[, .(eur_dia = sum(ing_viv * share * FACTORCAL) /
                       sum(d2 * share * FACTORCAL)), by = uso]
  ...
  barras(v, etq,
         ...
         sub = "Variable: suma de ingresos (c.102 al 100%) / suma de dias censales por referencia catastral (c.101 agregada); euros por dia")
}))
